...
function parsePnpmLockfile(content) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Parse packages section - collect all packages from lockfile
    if (inPackages && line.startsWith('  ') && !line.startsWith('    ') && trimmed.endsWith(':')) {
      const pkgMatch = trimmed.match(/^'?([^'@]+(?:@[^']+)?)'?:\s*$/);
      if (pkgMatch) {
        const pkgKey = pkgMatch[1];
        // Extract version from key (everything after last @)
        const lastAt = pkgKey.lastIndexOf('@');
        const version = lastAt > 0 ? pkgKey.substring(lastAt + 1) : '0.0.0';
        const name = pkgKey.split('/').pop();
