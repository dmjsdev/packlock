...
function buildGraph(lockfileData, sizes, auditData) {
  ...
  if (rootPkg) {
    ...
    addDependencyEdges(rootPkg, rootNode.id, packages, sizes, vulnerabilities, nodes, edges, nodeMap, false);
    
    // Add edges from root to dev dependencies
    if (rootPkg.devDependencies) {
      addDependencyEdges({ dependencies: rootPkg.devDependencies }, rootNode.id, packages, sizes, vulnerabilities, nodes, edges, nodeMap, true);
    }
  }
  ...
  function addDependencyEdges(pkg, sourceId, packages, sizes, vulnerabilities, nodes, edges, nodeMap, isDev) {
    ...
    for (const [depName, depVersion] of Object.entries(deps)) {
      ...
      if (targetId === undefined) {
        ...
        // Recursively add dependencies of this package
        if (depPkg && depPkg.dependencies) {
          addDependencyEdges(depPkg, targetId, packages, sizes, vulnerabilities, nodes, edges, nodeMap, isDev);
        }
      }
